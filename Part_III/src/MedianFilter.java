 ...
 * Date:     (Created - 23/MAR/2021), (Finalized - 02/APR/2021)
 * Logic:    An image is imported from a RemoteService, following the RMI protocols. The image arrives as a set of bytes
 *           and is then turned into a proper BufferedImage to be used for the entire process. The image is first
...
class MedianFilter {
    ...
    public static void main(String[] args) throws IOException, NotBoundException {
        ...
        System.out.println("Starting Concurrent RMI Median Process ...");

        //Executing the method in the main class, timing how long it takes to get a good overview of the time distribution.
        long startTime = System.nanoTime();

        //The client looks for the server object and registers for it, the server returns the stub and they can communicate.
        Registry registry = LocateRegistry.getRegistry("localhost", PORT);
        RemoteInterface service = (RemoteInterface) registry.lookup("//localhost/MedianService");
        BufferedImage img = ImageIO.read(new ByteArrayInputStream(service.extractBytes()));
